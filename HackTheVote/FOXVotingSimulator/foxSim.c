...
struct Candidate* createCandidate(char* name, char* slogan) {
    ...
    strncpy(can->name, name, 100); // // doesn't copy null byte if none, injecting 100 string could allow us to leak data later on
    can->isCandidate = 1;
...
struct Person* createPerson(char* name) {
    ...
    strncpy(person->name, name, 100); // doesn't copy null byte if none, injecting 100 string could allow us to leak data later on
...
void voteLoop() {
    ...
    while (1) {
        ...
        p = mapGet(buff); // we can retrieve another candidate's struct right there by finding hash collisions
        if (p==NULL) {
            p = createPerson(buff);
            mapPut(buff,p);
